...
use voiceos_core::{
    FixtureSleepProposalGenerator, RoutedSleepProposalGenerator, SleepConfig, SleepError,
    SleepProposalGenerator,
};
...
async fn run(
    ...
    state
        ...
        .append_execution_event(
            ...
            &actor,
            json!({"cycle":&result.0,"report":&result.1}),
        )
        .map_err(|error| api_error(StatusCode::INTERNAL_SERVER_ERROR, error.to_string()))?;
    Ok((
        StatusCode::CREATED,
        Json(json!({"cycle":result.0,"report":result.1})),
    ))
}
